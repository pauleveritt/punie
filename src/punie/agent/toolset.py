...
ACPToolset exposes ACP Client methods as Pydantic AI tools. Phase 3.3 implements
all core tools: read_file, write_file (with permission), run_command (with permission),
and terminal lifecycle tools.
"""
...
async def write_file(ctx: RunContext[ACPDeps], path: str, content: str) -> str:
    ...
    if perm_response.outcome.outcome != "selected":
        # Denied - clean up and return message
        ctx.deps.tracker.forget(tool_call_id)
...
def create_toolset() -> FunctionToolset[ACPDeps]:
    ...
    to ACP Client Protocol. Phase 3.3 includes all 7 tools:
    - read_file
