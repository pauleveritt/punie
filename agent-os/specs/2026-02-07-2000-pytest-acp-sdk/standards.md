# Standards: Agent Verification

This spec applies the **agent-verification** standard for all verification steps.
...
Task 6 (Verify) explicitly follows this standard:
1. Run `uv run pytest -v` directly (not `just test`)
2. Use `astral:ruff` skill for linting
3. Use `astral:ty` skill for type checking

All verification work uses Astral skills for guidance and diagnostics.
