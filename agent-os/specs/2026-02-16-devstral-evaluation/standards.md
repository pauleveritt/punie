# Standards for Devstral Small 2 Evaluation

The following standards apply to this research work.
...
This is research/evaluation work, not production code. The agent-verification standard applies to verification steps in each gate:

- **Gate 1:** Type checking smoke test scripts
- **Gate 2:** Linting benchmark scripts
- **Gate 3:** Checking zero-shot test scripts
- **Gate 4:** Verifying conversion and test scripts
- **Gate 5:** Validating full pipeline scripts

No other standards apply as this is exploratory research focused on model evaluation, not production feature development. The gated approach minimizes investment before committing to full conversion.

## Verification Commands for This Work

For all scripts created during evaluation:
...
uv run pytest scripts/ -k gate
```

**Never use:** `just test`, `just lint`, or `just typecheck` for verification of evaluation scripts.
