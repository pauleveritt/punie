...
async def generate_training_example(
    ...
    try:
        ...
        # Add 300 second (5 min) timeout for 30B model (slower than expected)
        ...
        if result.all_messages():
            for msg in result.all_messages():
                ...
                # Extract tool calls and results using part_kind discriminator
                    ...
                    for part in msg.parts:
                        ...
                        if part_kind == "tool-call":
                            ...
                            tool_calls.append({
                                ...
                                "result": None,  # Will be filled by matching tool-return
                            })
                        elif part_kind == "tool-return":
                            # This is a tool result - match it to the most recent call with same tool_name
                            for tc in reversed(tool_calls):
                                if tc["result"] is None and tc["tool"] == part.tool_name:
        ...
        print(f"⏱️  Timeout (300s) generating example for '{query}'")
        return None
...
async def generate_dataset(
    ...
    # Setup
    workspace = Path.cwd()
    # Use openai: prefix with OPENAI_BASE_URL env var (set by caller)
    # The model name matches what mlx_lm.server reports in /v1/models
    model_name = "openai:mlx-community/Qwen3-Coder-30B-A3B-Instruct-4bit"
...
async def main():
    ...
    await generate_dataset(
        ...
        target_count=10,  # Test with 20 first
        batch_size=1,  # 1 at a time for 30B (too slow for parallel)
    )
