#!/usr/bin/env bash
# Phase 33 overnight training pipeline
# Runs: train → fuse → quantize → smoke-test
# Logs to: logs/phase33_training.log
#
# Usage:
#   bash scripts/run_phase33_overnight.sh
#   nohup bash scripts/run_phase33_overnight.sh &

set -euo pipefail

LOGFILE="logs/phase33_training.log"
BASE_MODEL="mlx-community/Qwen3-Coder-30B-A3B-Instruct-4bit"
ADAPTER_PATH="adapters_phase33"
FUSED_F16="fused_model_qwen3_phase33_f16"
FUSED_5BIT="fused_model_qwen3_phase33_5bit"
...
log "======================================================"
log "Phase 33 Overnight Training Pipeline"
log "Base model: $BASE_MODEL"
log "Data: $DATA (train + valid)"
...
log "  Command: uv run python -m mlx_lm lora ..."
...
# ------------------------------------------------------------------ #
# STEP 5: Run Phase 33 eval suite
# ------------------------------------------------------------------ #
log ""
log "STEP 5: Running Phase 33 eval suite (26 prompts)"

EVAL_EXIT=0
uv run python scripts/run_phase33_eval.py --model "$FUSED_5BIT" --port 8080 \
    2>&1 | tee -a "$LOGFILE" || EVAL_EXIT=$?

if [ $EVAL_EXIT -eq 0 ]; then
    log "STEP 5 complete: eval PASSED ✓"
else
    log "STEP 5 complete: eval did not reach ≥80% target — check logs/phase33_eval_results.json"
fi
...
log ""
log "======================================================"
log "Phase 33 training pipeline COMPLETE"
...
log "======================================================"
log ""
log "Results: logs/phase33_eval_results.json"
log "Log:     $LOGFILE"
